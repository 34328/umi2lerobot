...
def run_server(
    ...
    def show_episode(dataset_namespace, dataset_name, episode_id, dataset=dataset, episodes=episodes):
        ...
        dataset_version = str(dataset.meta._version) if isinstance(dataset, LeRobotDataset) else dataset.codebase_version
...
def get_episode_data(dataset: LeRobotDataset | IterableNamespace, episode_index):
    ...
    for column_name in selected_columns:
        ...
        if "names" in dataset.features[column_name] and dataset.features[column_name]["names"]:
            ...
            # 处理嵌套列表的情况，例如 [['x', 'y', 'z', ...]] -> ['x', 'y', 'z', ...]
            while isinstance(column_names, list) and len(column_names) == 1 and isinstance(column_names[0], list):
...
def main():
    ...
    # 如果没有指定 repo_id 但有 root，尝试从 root 路径推断 repo_id
    if not repo_id and root and Path(root).exists():
        parts = Path(root).parts
        if len(parts) >= 2:
            repo_id = f"{parts[-2]}/{parts[-1]}"
            logging.info(f"Auto-detected repo_id from root path: {repo_id}")
    ...
    if repo_id:
        if load_from_hf_hub:
            ...
            # 加载本地数据集
            # root 直接指向数据集目录，使用 revision 参数跳过网络验证
            try:
                dataset = LeRobotDataset(repo_id, root=root, revision="v2.1")
                logging.info(f"Successfully loaded dataset: {repo_id} from {root}")
            except Exception as e:
                logging.warning(f"Failed to load dataset with v2.1, trying v2.0: {e}")
                try:
                    dataset = LeRobotDataset(repo_id, root=root, revision="v2.0")
                    logging.info(f"Successfully loaded dataset: {repo_id} from {root}")
