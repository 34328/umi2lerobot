...
class UmiDataset:
    ...
    def _parse_images(self, root, start_idx: int, end_idx: int) -> dict[str, np.ndarray]:
        ...
        for camera_key, zarr_key in self.camera_to_image_key.items():
            # camera_key: lerobot中的字段名 (e.g., 'camera0_rgb', 'left_tactile')
            # zarr_key: zarr中的数据键名 (e.g., 'camera0_rgb', 'camera0_left_tactile')
    ...
    def get_item(self, index: int) -> dict:
        ...
        # Load camera images (包括 RGB 和触觉图像)
...
def populate_dataset(
    ...
    for j in tqdm.tqdm(range(len(umi_dataset))):
        episode = umi_dataset.get_item(j)
        ...
        for i in range(num_frames):
            ...
            for pose_key, pose_array in demo_poses.items():
                frame[f"observation.state.{pose_key}"] = pose_array[i]
...
class ArgsConfig:
    ...
    # 数据路径相关
    raw_dir: Path = Path("/home/unitree/桌面/umi2lerobot/rawData/ViTaMIn/Test_Tube_Reorientation_dataset.zarr.zip")
    """原始 zarr 文件路径或目录"""
    
    project: str = "ViTaMIn"
    """项目名称 - 用于组织数据集"""
    
    subtask: str = "Test_Tube_Reorientation"
    """子任务名称 - 用作数据集名称"""
    
    # EEF 6Dof + Vision 信息配置
    robot_type: str = "ViTaMIn"
    """机器人类型 - 必须在 constants.py 的 ROBOT_CONFIGS 中定义"""

    # 文本表述 
    text: str = "Test Tube Reorientation"
    """人类指令"""
