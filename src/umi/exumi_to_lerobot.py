...
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "lerobot" / "src"))  # -> project_root/lerobot/src
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "tools"))  # -> project_root/tools
...
class ExUmiDataset:
    ...
    │   └── tactile_combined (N, 460, 680, 3) uint8
    └── meta
        └── episode_ends (M,) int64
    """
    ...
    def __init__(self, data_dirs: Path, robot_type: str, task_text: str | None = None) -> None:
        ...
        self.demo_pose_sensors = self.robot_config.demo_pose_sensors
    ...
    def _extract_umi_data(self, root, start_idx: int, end_idx: int, data_names: list[str]) -> np.ndarray:
        ...
        ExUMI 只有单臂 (robot0)，直接拼接 eef_pos, eef_rot_axis_angle, gripper_width
        ...
        Returns:
            Concatenated numpy array of shape (episode_length, 7)
    ...
    def _extract_demo_poses(self, root, start_idx: int, end_idx: int) -> dict[str, np.ndarray]:
        """
        Extract demo pose data for ExUMI.
        ...
        Returns:
            Dictionary mapping pose name to numpy array
        """
        demo_poses = {}
        
        for zarr_key, lerobot_key in self.demo_pose_sensors.items():
            # ExUMI demo poses have robot0_ prefix
            full_key = f"robot0_{zarr_key}"
            if full_key in root['data']:
                data = root['data'][full_key][start_idx:end_idx]
                demo_poses[lerobot_key] = data.astype(np.float32)
    ...
    def _parse_images(self, root, start_idx: int, end_idx: int) -> dict[str, np.ndarray]:
        ...
        for camera_key, image_key in self.camera_to_image_key.items():
            ...
            zarr_key = image_key  # e.g., 'camera0_rgb', 'tactile_combined'
    ...
    def get_item(self, index: int) -> dict:
        ...
        # Extract demo poses
        demo_poses = self._extract_demo_poses(root, start_idx, end_idx)
...
def create_empty_dataset(
    ...
    *,
    ...
    """Create an empty LeRobot dataset for ExUMI data.
    
    Args:
        image_shapes: Dict mapping camera name to (height, width, channels).
                      e.g., {"camera0_rgb": (224, 224, 3), "tactile_combined": (460, 680, 3)}
    """
    robot_config = ROBOT_CONFIGS[robot_type]
    motors = robot_config.motors
    cameras = robot_config.cameras
    demo_pose_sensors = robot_config.demo_pose_sensors
    demo_pose_shapes = robot_config.demo_pose_shapes

    # Default image shapes if not specified - 根据配置动态生成
    if image_shapes is None:
        default_shapes = {
            "camera0_rgb": (224, 224, 3),
            "tactile_combined": (460, 680, 3),
        }
        # 只保留配置中存在的相机
        image_shapes = {cam: default_shapes.get(cam, (224, 224, 3)) for cam in cameras}
    ...
    # Add camera features with their specific shapes
    for cam in cameras:
        cam_shape = image_shapes.get(cam, (224, 224, 3))
    ...
    # Add demo pose features
    for pose_name, shape in demo_pose_shapes.items():
        features[f"observation.{pose_name}"] = {
            "dtype": "float32",
            "shape": shape,
            "names": None,
        }
...
def populate_dataset(
    ...
    exumi_dataset = ExUmiDataset(raw_dir, robot_type, task_text)
    
    for i in tqdm.tqdm(range(len(exumi_dataset))):
        episode = exumi_dataset.get_item(i)
        ...
        for j in range(num_frames):
            ...
            # Add demo poses
            for pose_name, pose_array in demo_poses.items():
                frame[f"observation.{pose_name}"] = pose_array[j]
...
def exumi_to_lerobot(
    ...
    """
    Convert ExUMI dataset to LeRobot format.
    ...
    Example: ~/.cache/huggingface/lerobot/exumi/task_name
    """
...
class ArgsConfig:   
    ...
    # 数据路径相关
    raw_dir: Path = Path("/home/unitree/桌面/umi2lerobot/rawData/exUMI/pick_cube.zarr.zip")
    """原始 zarr 文件路径或目录"""
    
    project: str = "exUMI"
    """项目名称 - 用于组织数据集"""
    
    subtask: str = "pick_cube"
    """子任务名称 - 用作数据集名称"""
    
    # EEF 6Dof + Vision 信息配置
    robot_type: str = "ExUMI"
    """机器人类型 - 必须在 constants.py 的 ROBOT_CONFIGS 中定义"""

    # 文本表述 
    text: str = "pick up the cube"
    """人类指令"""
    ...
    # 图像形状配置 - ExUMI 有两种不同大小的图像
    camera0_shape: tuple[int, int, int] = (224, 224, 3)
    """camera0_rgb 图像形状 (高度, 宽度, 通道数)"""
    
    tactile_shape: tuple[int, int, int] = (460, 680, 3)
    """tactile_combined 图像形状 (高度, 宽度, 通道数)"""
...
if __name__ == "__main__":
    ...
    # 执行转换 (image_shapes=None 会根据配置自动生成)
    exumi_to_lerobot(
